...
def mock_speak(text: str):
    ...
    # Strip english characters because the el-GR TTS model will fail or spell them out awkwardly
    cleaned_text = re.sub(r'[a-zA-Z]', '', text).replace('-', ' ').replace('*', '').replace('#', '').replace('"', '').replace("'", "")
    if not cleaned_text.strip(): 
        cleaned_text = "Συγγνώμη, δεν το κατάλαβα."
    
    try:
        subprocess.run(["edge-tts", "--voice", "el-GR-NestorasNeural", "--text", cleaned_text, "--volume=+10%", "--write-media", mp3_filename], check=True, timeout=30)
