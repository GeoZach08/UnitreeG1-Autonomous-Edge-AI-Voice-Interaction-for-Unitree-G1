...
Όταν το ρομπότ βρίσκεται σε κατάσταση Standby (Μπλε), ο χρήστης μπορεί να επανεκκινήσει τη συζήτηση προφέροντας το Wake Word. Το σύστημα αναγνωρίζει λέξεις-κλειδιά ("μπαμπ", "barbie", "μπαρμπι") και παραφθορές αυτών. Εφόσον το Wake Word αναγνωριστεί, τα LED γίνονται πράσινα και το ρομπότ περιμένει την ερώτηση.
...
2.  **Διαχείριση Δρομολόγησης Ήχου (PulseAudio Routing):** 
    Η αρχιτεκτονική του ρομπότ διαχωρίζει το υλικό ήχου (ηχείο/μικρόφωνα στο PC1) από τη μονάδα επεξεργασίας (Jetson στο PC2)[cite: 1]. Χρησιμοποιήθηκε ένα custom bridge (G1 Audio Driver) που γεφυρώνει τα PCs μέσω Multicast UDP και DDS PlayStream RPC, δημιουργώντας τις εικονικές συσκευές `g1_microphone` και `g1_speaker`[cite: 1]. Για την αποφυγή λανθασμένης δρομολόγησης, προστέθηκαν εντολές συστήματος (`pactl set-default-sink/source`) εντός του κώδικα Python[cite: 1] και χρησιμοποιήθηκε η μεταβλητή `PULSE_SINK` στο subprocess του `ffplay`. Ο ήχος χρησιμοποιεί αυστηρά το format `s16le` (16,000 Hz, 1 κανάλι, 16-bit)[cite: 1].
...
4.  **Κλείδωμα Μικροφώνου (Hardware Privacy):** 
    Το μικρόφωνο του ρομπότ παραμένει απενεργοποιημένο από το εργοστασιακό υλικολογισμικό[cite: 1]. Απαιτείται η ρητή ενεργοποίηση του "Voice Assistant" μέσω της επίσημης εφαρμογής της Unitree για την εκκίνηση της ροής δεδομένων (αλλάζοντας το mode μέσω του API 1008)[cite: 1].
...
Απαιτείται ο εγκαταστάτης του custom οδηγού ήχου (PulseAudio bridge) για τη δρομολόγηση των μικροφώνων και του ηχείου[cite: 1].
```bash
...
./install.sh --start
```
*(Απαιτείται η ενεργοποίηση Multicast στο τοπικό δίκτυο της θύρας `eth0` στο υποδίκτυο `192.168.123.0/24`, καθώς ο οδηγός λαμβάνει δεδομένα UDP στη διεύθυνση `239.168.123.161:5555`)[cite: 1].*

### Βήμα 3: Εγκατάσταση Unitree SDK2 (Μέσα στο Ρομπότ)
1. Κατεβάστε το `unitree_sdk2_python` από την Unitree στον κατάλογο του Jetson (προεπιλογή: `~/unitree-sdk2-python`)[cite: 1].
...
2. Στην εφαρμογή Unitree Explore, ενεργοποιήστε το **"Voice Assistant"**. *(Χωρίς αυτό το βήμα, το hardware μικρόφωνο δεν μεταδίδει δεδομένα)*[cite: 1].
...
Για πλήρη αυτονομία χωρίς ανάγκη SSH, το σύστημα έχει ρυθμιστεί να εκκινείται αυτόματα κατά την εκκίνηση του ρομπότ[cite: 1]. Δημιουργήστε ένα user service αρχείο:
```bash
